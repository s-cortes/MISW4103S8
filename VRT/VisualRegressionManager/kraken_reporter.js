...
const reportsDir = './resultsKraken/';

function createReport() {
    const specificReport = argv[2] === 'results-from';
    if(specificReport) {
        const reportsFrom = argv[3];
        console.log(`KrakenReportes: Reporting for ${reportsFrom}...`);
        createReportForSpecificDate(reportsFrom);
        console.log('Done!');
    } else {
        console.log(`KrakenReportes: Reporting for all Testing Dates`);
...
function createScenarioReport(reportData){
    ...
    <html>
        <head>
            <title> Kraken VRT Report </title>
            <link href="index.css" type="text/css" rel="stylesheet">
        </head>
        <body>
            <h1>Report for Kraken Tests on Ghost versions</h1>
